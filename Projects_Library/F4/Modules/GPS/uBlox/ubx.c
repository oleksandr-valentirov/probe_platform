...
static void UBX_CalcChecksum(size_t payload_size)
{
    ...
    uint8_t* CK_0 = ((uint8_t* )tx_packet_start) + payload_size;
    uint8_t* CK_1 = CK_0 + 1;
    *CK_0 = 0;
    *CK_1 = 0;
...
uint8_t UBX_SetMsgRate(uint8_t cls, uint8_t id, uint8_t rate)
{
    ...
    DMA_GPSoutTransfer(header.length);
    return 0;
}
...
{    
    ...
    uint8_t nmea_msg_id[18] = {NMEA_DTM_ID, NMEA_GBQ_ID, NMEA_GBS_ID,
        NMEA_GGA_ID, NMEA_GLL_ID, NMEA_GLQ_ID, NMEA_GNQ_ID, NMEA_GNS_ID,
        NMEA_GPQ_ID, NMEA_GRS_ID, NMEA_GSA_ID, NMEA_GST_ID, NMEA_GSV_ID,
        NMEA_RMC_ID, NMEA_THS_ID, NMEA_VLW_ID, NMEA_VTG_ID,
        NMEA_ZDA_ID
    };
    
    for(uint8_t i = 0; i < 18; i++)
    {
        while(UBX_SetMsgRate(NMEA_CLASS, nmea_msg_id[i], 0));
        // wait for response
        // process response
    }
