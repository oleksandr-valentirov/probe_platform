...
{
    ClearReadyFlag;
    USART2_Start_Transmission("AT+CSQ\r\n", 8);
}
...
void FlyMode(FunctionalState state)
{
    ...
    ClearReadyFlag;
    USART2_Start_Transmission(cmd, 11);
}
...
static void Sim_TokenCMD(FunctionalState state)
{
    ...
    if(USART2_Start_Transmission(msg, 12) == 0)
    {
        Sim_SAPBREventStart();
    }
}
...
static void Sim_SendSMSCmd(void)
{
    ...
    if(USART2_Start_Transmission(cmd, 24) == 0)
    {
        ClearCallFlag;
    }
}
...
static void Sim_SendMsg(void)
{
    ...
    if(USART2_Start_Transmission(msg, 46) == 0)
    {
        ClearMsgTxtInFlag;
    }
}
...
static void Sim_ReceiveCall(void)
{
    ...
    if(USART2_Start_Transmission("ATH\r\n", 5) == 0)
    {
        Sim_ATHEventStart();
...
static void Sim_gets(void)
{
    ...
    while(buf_pos < SIM_BUF_SIZE && USART2_getc(&c))
